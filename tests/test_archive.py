...
def test_nhan_ra_file_da_nap(conn, tmp_path):
    ...
    archive.store(conn, OK, "zaiko", d, 177, 137_839_071, tmp_path, date(2026, 9, 16))
...
def test_previous_stats(conn, tmp_path):
    archive.store(conn, OK, "zaiko", "aaa", 177, 137_839_071, tmp_path, date(2026, 9, 16))
...
def test_undo_khong_xoa_dong(conn, tmp_path):
    d = archive.sha256_of(OK)
    bid = archive.store(conn, OK, "zaiko", d, 177, 137_839_071, tmp_path, date(2026, 9, 16))
...
def test_hoan_tac_roi_nap_lai_duoc(conn, tmp_path):
    ...
    archive.undo(conn, bid)
    assert archive.already_loaded(conn, d) is False
    bid2 = archive.store(conn, OK, "zaiko", d, 177, 137_839_071, tmp_path, date(2026, 9, 16))   # KHÔNG được crash
    assert bid2 != bid
    assert archive.already_loaded(conn, d) is True

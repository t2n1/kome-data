...
| Đối chiếu bắt buộc | ảnh chụp ← `kome/gates.py`; lúc chạy ← `files.yml` | Tên cổng: regex dòng chú thích `# Cổng N — <tên>` trong `gates.py` (lấy lần xuất hiện ĐẦU của mỗi N). Loại chặn/cảnh báo: cổng 1–3 chặn, 4–5 cảnh báo (docstring của `check`). Ngưỡng từng file: `min_rows`, `warn_row_drop_ratio`, `warn_total_spike`, `warn_total_drop`, `product_check`, `required_date_columns`, `dedup_on_keys` |
